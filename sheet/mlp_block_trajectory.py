...
class MlpBlockTrajectory(nn.Module):
    def __init__(self, config: SheetGeometryConfig, *, runtime_dtype: torch.dtype = torch.float32, basis_version: str = BASIS_VERSION, basis_family: str = BASIS_FAMILY_CHEBYSHEV) -> None:
        ...
        self.mlp_metadata = (
            ...
            MlpBlockMetadata(MLP_CONTRACTION_WEIGHT, "matrix", "mlp_block_matrix_normal", residual_std, True, width, 4 * width, 4 * width, derive_row_order(width, width, config.base_row_order), derive_row_order(4 * width, width, config.base_row_order)),
        )
        ...
        for name, parameter in self.curve.coefficients.items():
            self.coefficients[name] = parameter
        for item in self.mlp_metadata:
    ...
    def family_metadata(self, name: str):
        if name in self._mlp_metadata_by_name:
            return self._mlp_metadata_by_name[name]
    ...
    def reset_parameters(self) -> None:
        ...
        with torch.no_grad():
            for item in self.mlp_metadata:
                ...
                coefficient.zero_()
                coefficient_std = item.target_weight_std * math.sqrt(self.config.n_layer * item.output_rows * item.row_width / (item.output_order * item.input_order))
    ...
    def _materialize_mlp_block(self, name: str, layer_index: int) -> Tensor:
        item = self._mlp_metadata_by_name[name]
        ...
        output_basis = self.output_basis(name).to(device=coefficient.device, dtype=coefficient.dtype)
        input_basis = self.input_basis(name).to(device=coefficient.device, dtype=coefficient.dtype)
        generated = output_basis @ mixed @ input_basis.transpose(0, 1)
        if tuple(generated.shape) != (item.output_rows, item.row_width):
            raise RuntimeError(f"bad mlp_block shape for {name}: {tuple(generated.shape)}")
        return generated

    def materialize(self, name: str, layer_index: int) -> Tensor:
        if name in self._mlp_metadata_by_name:
            return self._materialize_mlp_block(name, layer_index)
