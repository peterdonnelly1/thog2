...
class MlpBlockTrajectory(nn.Module):
    """MLP matrix-axis blocks over DEPTH attention and vector fallbacks."""
    ...
    def __init__(
        ...
        *,
        ...
        del self.depth.coefficients[MLP_CONTRACTION_WEIGHT]
        width = config.n_embd
        model_order = config.resolved_o_mlp_d_model
        mlp_order = config.resolved_o_mlp_hidden
        self.mlp_metadata = (
            MlpBlockMetadata(
                MLP_EXPANSION_WEIGHT,
                "matrix",
                "mlp_block_matrix_normal",
                0.02,
                True,
                4 * width,
                width,
            ...
            MlpBlockMetadata(
                ...
                "matrix",
                "mlp_block_matrix_normal",
                0.02 / math.sqrt(2.0 * config.n_layer),
                True,
                width,
                4 * width,
                4 * width,
        ...
        self.metadata = tuple(item for item in self.depth.metadata if item.name not in {MLP_EXPANSION_WEIGHT, MLP_CONTRACTION_WEIGHT}) + self.mlp_metadata
    ...
    def family_metadata(self, name: str):
        return self._mlp_metadata_by_name[name] if name in self._mlp_metadata_by_name else self.depth.family_metadata(name)
    ...
    def _reset_mlp_parameters(self) -> None:
        with torch.no_grad():
            for item in self.mlp_metadata:
                ...
                coefficient.zero_()
                std = item.target_weight_std * math.sqrt(
                    self.config.n_layer * item.output_rows * item.row_width / (item.output_order * item.input_order)
                )
    ...
    def materialize(self, name: str, layer_index: int) -> Tensor:
        return self._materialize_mlp_block(name, layer_index) if name in self._mlp_metadata_by_name else self.depth.materialize(name, layer_index)
