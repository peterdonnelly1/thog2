...
RESIDUAL_INIT_DEPTH_SOURCES = ("logical_depth", "basis_depth", "constant")
DEFAULT_RESIDUAL_INIT_POLICY = "depth_scaled"
DEFAULT_RESIDUAL_INIT_DEPTH_SOURCE = "logical_depth"
...
class ResidualInitConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.depth_source not in RESIDUAL_INIT_DEPTH_SOURCES:
            raise ValueError(
                "residual init depth source must be one of "
                f"{RESIDUAL_INIT_DEPTH_SOURCES}; got {self.depth_source!r}"
            )
        if (
    ...
    def effective_depth(
        ...
        *,
        ...
        if self.depth_source == "logical_depth":
            return n_layer
        if self.depth_source == "basis_depth":
            ...
            if (
                ...
                raise ValueError(
                    "basis_depth residual init requires a positive depth_order; "
                    f"got {depth_order!r}"
                )
            return depth_order
        if self.depth_source == "constant":
