...
def build_model(config: TrainingConfig) -> nn.Module:
    ...
    with torch.random.fork_rng(devices=fork_devices):
        ...
        if config.model_type == "dense":
            model: nn.Module = GPT(GPTConfig(**config.model_arguments()))
        elif config.model_type == "thog2_sheet":
            model = SheetGPT(SheetGPTConfig(**config.model_arguments()))
        else:
            raise ValueError(f"unsupported model_type: {config.model_type}")
    return model.to(device)


def parameter_report(model: nn.Module, model_type: str) -> Dict[str, Any]:
