...
class OwtTrainer(Stage6Trainer):
    ...
    def _print_progress(self, run_id: str, event: str, **payload: Any) -> None:
        ...
        if "consumed_tokens" in values:
            values["consumed_tokens"] = int(values["consumed_tokens"]) * int(
...
def validate_dataset(dataset_dir: Path, block_size: int) -> Dict[str, Any]:
    ...
    if not train_path.is_file() or not validation_path.is_file():
        raise FileNotFoundError(
            f"dataset must contain train.bin and val.bin: {dataset_dir}"
        )
...
def validate_resume_controls(
    ...
    mismatches = [
        f"{name}: checkpoint={getattr(stored, name)!r}, requested={getattr(expected, name)!r}"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train or resume one canonical THOG2 OpenWebText run"
    )
...
def main() -> int:
    ...
    payload = resolved_payload(
        config,
        world_size=world_size,
        log_timestamp=arguments.log_timestamp,
    )
    ...
    try:
        ...
        result = trainer.run_pilot(
            run_id=config.artifact_name,
            protocol_sha256=run_digest(config, dataset, world_size),
            dataset=dataset,
