...
# vvv THOG
# Dreedle layer-dropout experiment matched to the stable REVAMPV1 GPU1 L32 DEPTH run.
export THOG2_DREEDLE_GPU="${THOG2_DREEDLE_GPU:-1}"
export CUDA_VISIBLE_DEVICES="$THOG2_DREEDLE_GPU"
export THOG2_HOST_LABEL="${THOG2_HOST_LABEL:-dreedle_gpu${THOG2_DREEDLE_GPU}_dropout}"
...
./train_OWT.sh \
  -g REVAMPv1_DREEDLE_DROPOUT_GPU${THOG2_DREEDLE_GPU}_LS4_LA3_LI10 \
  -n 10000 \
  -b 16 \
  -A 8 \
  -G "$THOG2_NUM_GPUS" \
