...
Bias-corrected layer moments determine the AdamW denominator and update. The actual production materializer's pseudoinverse then projects this update into the weight coefficients. Weight decay is decoupled and applied once. Candidate parameters and histories are prepared before commit. Weight candidates read their original device tiles directly. Candidate weights and histories stay on the GPU when the transient memory budget permits, otherwise they use the host transaction path. GPU candidate histories are copied into stable persistent buffers on the device, avoiding a device-to-host-to-device round trip. Failed candidates leave parameters, moments and counters unchanged and use the trainer's existing nonfinite policy.

Persistent covered-family moments contain `(H_m+H_v)*N` values, compared with `2*L*N` for dense AdamW, where N is the number of coupling coordinates. Raw host staging still contains `L*N` values. Transaction candidates consume host or temporary device memory. Auto placement may increase the optimizer phase's peak GPU allocation; it does not add a permanent history replica. Updates that initialize any participating parameter's histories use host candidates. After validation and CUDA completion, unused cached blocks are released before allocating those histories. Subsequent updates reuse the same history storage and need no cache cleanup. Their candidate budget is the larger of released allocation-peak headroom and half of currently available memory (physical free plus unused allocator reserve), capped by availability, less workspace reserves. This aggregate estimate is not a guarantee against fragmentation or competing GPU allocations. Candidate scratch is released before the next forward pass. Device work is tiled, with a default of at most 1,048,576 layer values per individual tile tensor; several such tensors coexist during fitting. Persistent bases, model parameters and ordinary parameter candidates are additional allocations. This is a device-history saving, not an elimination of host storage.
...
With `DEBUG > 99` in `constants.py`, `THOGOPT last update:` reports `stage`, `prepare`, `candidate`, `commit`, `gpu_candidates` in MiB, and `async_staging=yes|no` at the existing console log cadence. At `DEBUG <= 99`, component timing clocks and measurement-only CUDA waits are disabled and the extra row is hidden; normal training progress remains visible. Synchronization needed for safe first-state allocation and gradient transfers remains active at every debug level. The optimizer reads the timing setting at construction, so restart/resume to change it. Diagnostic metrics include `timing_enabled`; zero timing placeholders when disabled are not measurements. The standalone benchmark reports those unavailable component times as null and still measures total and optimizer time independently.

These diagnostics describe the last completed update, not an average across the logging interval. `stage` measures CPU time spent scheduling/draining gradient copies and accumulating values; it includes waits for GPU work and is not total DMA duration or pure added optimizer overhead. `prepare` includes the final copy drain, unscale/reduction/norm/clipping. `candidate` and `commit` include CUDA completion waits. Forward/backward computation and instrumentation are outside these categories, so their sum is not whole-step time.
...
The next user run completed its first update in 5.6s, with 2905MiB of GPU candidates, then failed in backward on the next update. CUDA requested 2.30GiB with 1.58GiB free; PyTorch reported 7.81GiB allocated and 5.52GiB reserved but unallocated. One completed update does not establish sustained throughput. The model remained D=1024, L=P=H_m=H_v=16, batch16 and accumulation6.

The preceding commit adopted temporary GPU candidate histories as permanent optimizer state. That can pin portions of split allocator blocks and prevent their reuse by larger backward allocations. This is a code-level lifetime defect consistent with the reported memory figures; no allocator snapshot or GPU reproduction is available here. PyTorch describes unused cached memory and split-block fragmentation in its [CUDA memory documentation](https://docs.pytorch.org/docs/2.8/notes/cuda.html#memory-management).

The correction uses host candidates while initializing histories, releases unused cached CUDA blocks before allocating permanent state, then preserves those state buffers across updates. Ordinary AdamW calculation now has a separate function scope so its temporary tensors are gone before initial state allocation. Later GPU candidates copy into permanent history buffers on the GPU; asynchronous raw-gradient staging remains enabled. Initialization cleanup does not run on routine updates. A later first gradient for a previously untouched parameter also follows the initialization path.

Keep the same D=1024 model and A/B snapshots for the next C run. No optimizer reset or new flags are needed; stop/restart or resume to load the corrected code. With `DEBUG > 99`, a fresh run's first timing line should show `gpu_candidates=0MiB`; later lines can show nonzero GPU candidates. The first update now includes initialization work and should not be used as the steady-state speed measurement. If a further CUDA failure occurs, `THOG2_THOGOPT_TRANSACTION_STORAGE=host` disables device candidates while retaining asynchronous staging for diagnosis.

Regression coverage checks stable storage for both compressed and ordinary moments, candidate-history release after each commit, an injected allocation failure without partial weight/state mutation, numerical parity and resume. The CUDA test also checks host initialization, later GPU candidates, stable history addresses, one initialization cache release and atomic rejection while interleaving large temporary allocations. CUDA execution remains unavailable here. See [oom_followup_python_tests.txt](oom_followup_python_tests.txt) for the consolidated results; a representative GPU rerun is still required.

Subsequent user GPU evidence reached30 updates at D=1024 without repeating the OOM: initialization6.9s, then5.6-5.8s/update with2905MiB GPU candidates. A complete500-update outcome remains unreported. The later timing gate is covered by [debug_timing_tests.txt](debug_timing_tests.txt), including the99/100 threshold, numerical neutrality and checkpoint compatibility across timing settings.
