...
## 2026-08-21 — Workspace current weights and Weights group settings

- The Runs view correctly retained only DENSE step 336, while the same chart in Workspace retained older traces despite its Workspace-scoped `Current weights only` setting. The earlier smoke test exercised merge and filtering separately, not the complete final presentation chain.
- Chose a scoped group-default model: each run and Workspace have independent Weights settings; an individual chart can retain an explicit override or opt back into the applicable Weights group values.
- Fixed the optimizer-update fallback so an empty late Workspace-owner value no longer hides valid DENSE/THOG update metadata. Re-applied newest-step filtering after the complete presentation chain, retaining every scalar at each visible run's own latest step while dropping historical or unidentified traces owned by that run.
- Added the Weights header gear in both Runs and Workspace. It controls current-only mode, THOG integer-point joining, step count/window, outlier handling, smoothing, line width, chart type, grids, and the six-chart Y-scale choice. The group store is run/Workspace scoped; each chart exposes an explicit inheritance toggle and can override common group values without inheriting title or axis bounds.
- The semantic smoke now covers null-owner fallback, two runs at different latest steps, multiple scalars on one latest DENSE step, unidentified historical trace rejection, non-data anchor retention, and chart-over-group precedence. Every dashboard JavaScript asset passes `node --check`; the semantic smoke, Python compilation, static asset-order checks and scoped whitespace checks pass. The full Python dashboard module remains unavailable locally because this sandbox has neither Plotly nor pytest.

## 2026-08-23 — INSTRA weight-render contract regression repair

- Reproduced one shared cause for the reported THOG index, curve-shape and legend regressions: the late reliability layer restored a raw random trace after matched-coupling filtering and after the integer-segment transform.
- Replaced that fallback with an honest unavailable annotation. Current-only Runs and Workspace views now render the selected coupling or no coupling; they never substitute a differently indexed recorded pair.
- Consolidated weight-chart chrome in the presentation owner: the card/group headers own identity, while all six Plotly figures suppress their internal title, right-hand legend and duplicate top-axis title. Hover metadata remains intact.
- Kept the existing integer-segment transform as the sole THOG shape owner. Selected THOG data use exact integer coordinates, open-circle markers and linear connectors; DENSE retains its native crosses and straight connectors.
- Corrected server-side matched-selection annotation to key curves by optimizer update and scalar id. Historical traces can no longer inherit the newest snapshot's feature indices when a scalar id is reused.
- Made heatmap visibility data-driven as well as model-driven. Workspace and genuinely empty DENSE runs remain suppressed, but a run with stored heatmap rows or enabled heatmap capture cannot have the group hidden by its model label.
- Added semantic coverage for stale-coupling omission, exact selected segments, absent duplicate chrome, per-update metadata, heatmap availability, and a real-Firefox modern THOG render contract. All JavaScript syntax checks and Node regressions pass locally; the focused non-browser Python selection passes 161 tests plus 9 subtests. Real-Firefox and broad comparison verification remain owned by branch CI.
