...
- Corrected mixed-precision envelope pricing. The pre-block pass reference remains FP32 under CUDA autocast, but Premat matrices and overlapping projection activations are BF16/FP16; forcing four-byte estimates doubled both retained and foreground charges and created binary false rejections near the global-buffer boundary. Runtime admission now uses the active CUDA autocast element size, records it explicitly, and retains FP32 pricing when autocast is inactive. Scruffy's first BF16 equivalence run matched logits/loss and all gradient keys; its maximum deterministic gradient rounding difference was 1.22e-4, so BF16 comparison uses a bounded dtype-appropriate tolerance while FP32 remains strict.
