...
- Published the assertion diagnostics as `689220d7a7cfd86af07b130c158972657c0f3c75`. Hosted run `33136567112` reproduced the single Firefox failure and proved that gradient mode was pressed, both Workspace runs each retained all three requested steps, and every rendered trace still had its run's base colour.
- Traced the browser install order to the v0.58 Workspace compatibility layer: its delayed 1.25-second `prepare_figure` wrapper ran after the final gradient owner and unconditionally replaced the computed step colours. The repair preserves already-computed non-DENSE Workspace colours only while gradient mode is enabled; normal run colouring and DENSE history colouring are unchanged. Added a deterministic late-wrapper regression for line, marker, and marker-outline colours.
- Published the exact repaired implementation tree as `07d513db52cd04a4e8ab3e2a761421ffbee66226` (tree `d4d10b6d21d382998108fbabf7f04be94b4227fb`). Hosted run `33136990508` passed compilation, all deterministic JavaScript checks, the 207-test focused suite plus 9 subtests including real Firefox, and the broad 219-file head-versus-`chaos_bump_sampling` comparison with zero new regressions. Comparison artifact: `depth-weight-regression-07d513db52cd04a4e8ab3e2a761421ffbee66226` (`9672673350`).
