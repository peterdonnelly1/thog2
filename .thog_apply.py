...
def update_wrapper(path: Path) -> None:
    ...
    loop_prefix = 'if (( ${#PRESET_VALUES[@]} > 1 || ${#O_DEPTH_VALUES[@]} > 1 ||'
    loop_start = next(
        (index for index, line in enumerate(output) if line.startswith(loop_prefix)),
        None,
    )
    if loop_start is None:
        raise RuntimeError(f"{path}: could not find old grid loop")
...
'''
    ...
    if count != 1:
        raise RuntimeError(f"{path}: expected one fake tag-print line; found {count}")
