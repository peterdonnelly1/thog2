...
# vvv THOG apply requested wrapper defaults, display changes, LR ranges, and core recovery policy
wrappers = (
...
for path in (Path("sheet/training_config.py"), Path("sheet/run_config.py")):
    ...
    path.write_text(
        text.replace(old, 'nonfinite_update_policy: str = "skip"', 1),
        encoding="utf-8",
    )
...
Path("tests/test_picton_wrapper_defaults_and_nonfinite_policy.py").write_text(
    '''# vvv THOG
