    ...
    while (($#)); do
        case "$1" in
            ...
            -h|--help)
                cat <<EOF
Usage:
  source scripts/setup_thog2_env.sh [--recreate]
...
EOF
    ...
    if [[ -n "${CONDA_PREFIX:-}" ]]; then
        if ! type conda >/dev/null 2>&1 && command -v conda >/dev/null 2>&1; then
...
EOF
    ...
    if [[ ! -x "$venv_dir/bin/python" ]]; then
        ...
        printf '  source scripts/setup_thog2_env.sh --recreate\n' >&2
        return 1
    else
...
Run:
  source scripts/setup_thog2_env.sh
EOF
    exit 2
fi
