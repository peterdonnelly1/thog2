...
class Stage6bIndependentHeadAwareBlockTests(unittest.TestCase):
    def attention_only_config(self) -> SheetGPTConfig:
        return SheetGPTConfig(block_size=8, vocab_size=32, n_layer=4, n_head=2, n_embd=16, dropout=0.0, bias=True, depth_order=3, base_row_order=8, attention_geometry=ATTENTION_GEOMETRY_HEAD_AWARE_BLOCK, mlp_geometry=MLP_GEOMETRY_DEPTH)

    def test_01_explicit_head_aware_block_attention_with_depth_mlp_resolves_to_attention_only_ablation_identity(self) -> None:
        config = stage4_training_config(attention_geometry=ATTENTION_GEOMETRY_HEAD_AWARE_BLOCK, mlp_geometry=MLP_GEOMETRY_DEPTH)
        ...
        full_block_identity = stage4_training_config(geometry_preset=GEOMETRY_PRESET_FULL_BLOCK).compact_identity_metadata()
        self.assertEqual(full_block_identity["attention_geometry"], ATTENTION_GEOMETRY_HEAD_AWARE_BLOCK)
        self.assertEqual(full_block_identity["mlp_geometry"], MLP_GEOMETRY_MLP_BLOCK)

    def test_02_independent_head_aware_block_trajectory_compacts_attention_per_head_but_leaves_mlp_on_depth_coefficients(self) -> None:
    ...
    def test_03_independent_head_aware_block_parameter_report_is_between_depth_and_full_block_because_only_attention_is_blocked(self) -> None:
        torch.manual_seed(4101)
        attention_only = SheetGPT(self.attention_only_config())
        depth = SheetGPT(SheetGPTConfig(**{**self.attention_only_config().to_dict(), "geometry_preset": GEOMETRY_PRESET_DEPTH, "attention_geometry": None, "mlp_geometry": None}))
        full_block = SheetGPT(SheetGPTConfig(**{**self.attention_only_config().to_dict(), "geometry_preset": GEOMETRY_PRESET_FULL_BLOCK, "attention_geometry": None, "mlp_geometry": None}))
        attention_only_count = attention_only.parameter_report()["matrix_sheet_coefficients"]
        depth_count = depth.parameter_report()["matrix_sheet_coefficients"]
        full_block_count = full_block.parameter_report()["matrix_sheet_coefficients"]
        self.assertLess(attention_only_count, depth_count)
    ...
    def test_04_independent_head_aware_block_checkpoint_round_trips_and_rejects_full_block_or_mlp_block_cross_loads(self) -> None:
        ...
        attention_only_config = stage4_training_config(attention_geometry=ATTENTION_GEOMETRY_HEAD_AWARE_BLOCK, mlp_geometry=MLP_GEOMETRY_DEPTH, max_updates=1)
        full_block_config = stage4_training_config(geometry_preset=GEOMETRY_PRESET_FULL_BLOCK, max_updates=1)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            full_block_path = Path(directory) / "full_block.pt"
    ...
    def test_05_independent_head_aware_block_tiny_train_step_runs_with_depth_mlp_fallback(self) -> None:
        train_tokens, validation_tokens = stage4_tokens(128)
        config = stage4_training_config(attention_geometry=ATTENTION_GEOMETRY_HEAD_AWARE_BLOCK, mlp_geometry=MLP_GEOMETRY_DEPTH, max_updates=1)
