...
class ResumeAndForkCliCompatibilityTests(unittest.TestCase):
    @staticmethod
    def _preserved_wrapper_text() -> str:
        return (Path(__file__).resolve().parents[1] / "train_OWT_core.sh").read_text(encoding="utf-8")

    def test_lifecycle_short_option_coverage_matches_preserved_master_wrapper(self) -> None:
        wrapper = self._preserved_wrapper_text()
    ...
    def test_lifecycle_parser_accepts_all_preserved_master_long_controls(self) -> None:
        wrapper = self._preserved_wrapper_text()
        ...
        self.assertTrue(
            master_long_options <= lifecycle_long_options,
