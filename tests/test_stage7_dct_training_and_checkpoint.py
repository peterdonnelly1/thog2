...
class Stage7DctTrainingAndCheckpointTests(unittest.TestCase):
    ...
    def test_02_dct_checkpoint_payload_records_dct_basis_version_and_resumes_with_the_same_identity(self) -> None:
        train_tokens, validation_tokens = stage4_tokens(256)
        config = stage4_training_config(geometry_preset=GEOMETRY_PRESET_BLOCK, basis_family=BASIS_FAMILY_DCT, max_updates=2, decay_updates=2)
        trainer = Stage4Trainer(config, train_tokens, validation_tokens)
        trainer.train_one_update()
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "dct_block.pt"
    ...
    def test_03_chebyshev_and_dct_checkpoint_cross_loads_hard_fail_even_when_geometry_and_shapes_match(self) -> None:
        train_tokens, validation_tokens = stage4_tokens(256)
        dct_config = stage4_training_config(geometry_preset=GEOMETRY_PRESET_CURVE, basis_family=BASIS_FAMILY_DCT, max_updates=1)
        cheby_config = stage4_training_config(geometry_preset=GEOMETRY_PRESET_CURVE, basis_family=BASIS_FAMILY_CHEBYSHEV, max_updates=1)
        ...
        with tempfile.TemporaryDirectory() as directory:
            dct_path = Path(directory) / "dct_curve.pt"
            cheby_path = Path(directory) / "cheby_curve.pt"
