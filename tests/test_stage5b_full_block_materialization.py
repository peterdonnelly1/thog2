...
class Stage5bFullBlockMaterializationTests(unittest.TestCase):
    ...
    def test_01_full_block_config_identity_is_accepted_and_dct_or_mixed_partial_block_modes_still_fail(self) -> None:
        ...
        for overrides in (
            {"geometry_preset": GEOMETRY_PRESET_BLOCK, "basis_family": BASIS_FAMILY_DCT},
    ...
    def test_02_full_block_trajectory_replaces_every_repeated_matrix_family_with_depth_output_and_input_block_coefficients(self) -> None:
        config = self.full_block_config()
        trajectory = BlockTrajectory(config.sheet_geometry(), runtime_dtype=torch.float32, basis_version=config.basis_version)
        expected_shapes = {
            ATTENTION_QUERY_WEIGHT: (3, 8, 8),
            ATTENTION_KEY_WEIGHT: (3, 8, 8),
            ATTENTION_VALUE_WEIGHT: (3, 8, 8),
            ATTENTION_OUTPUT_WEIGHT: (3, 8, 8),
    ...
    def test_03_full_block_materialization_matches_manual_depth_output_and_input_basis_contraction_for_attention_and_mlp(self) -> None:
        config = self.full_block_config()
        trajectory = BlockTrajectory(config.sheet_geometry(), runtime_dtype=torch.float32, basis_version=config.basis_version)
        with torch.no_grad():
            ...
            trajectory.coefficients[ATTENTION_OUTPUT_WEIGHT][:, 4, 5] = torch.tensor([-1.0, 0.5, 2.0])
            trajectory.coefficients[MLP_EXPANSION_WEIGHT][:, 6, 7] = torch.tensor([0.25, -0.5, 1.5])
        layer_index = 1
        for name, output_row, input_column in (
            (ATTENTION_QUERY_WEIGHT, 7, 5),
            (ATTENTION_OUTPUT_WEIGHT, 9, 11),
            (MLP_EXPANSION_WEIGHT, 13, 3),
        ):
            with self.subTest(name=name):
                depth = trajectory.depth_basis[layer_index]
                collapsed = torch.einsum("p,pab->ab", depth, trajectory.coefficients[name])
                manual = trajectory.output_basis(name) @ collapsed @ trajectory.input_basis(name).transpose(0, 1)
                materialized = trajectory.materialize(name, layer_index)
                torch.testing.assert_close(materialized, manual, rtol=0.0, atol=0.0)
                torch.testing.assert_close(trajectory.direct_value(name, layer_index, output_row, input_column), materialized[output_row, input_column], rtol=0.0, atol=0.0)
    ...
    def test_04_full_block_parameter_report_counts_all_six_matrix_families_as_block_coefficients_and_is_smaller_than_mlp_block(self) -> None:
        ...
        attention_block_coefficients = 4 * depth_order * 8 * 8
        ...
        self.assertEqual(report_by_name[ATTENTION_QUERY_WEIGHT]["coefficient_shape"], (3, 8, 8))
    ...
    def test_06_full_block_checkpoint_identity_round_trips_and_rejects_mlp_block_curve_or_legacy_cross_loads(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for payload, config in (
                (block_payload, mlp_block_config),
                (mlp_block_payload, block_config),
                (curve_payload, block_config),
                (legacy_payload, block_config),
            ):
