...
FIXTURE_PATH = (
    ...
    / "fixtures"
    / "stage0_legacy_sheet_col_fixture.json"
)
...
class Stage0LegacySheetColBaselineTests(unittest.TestCase):
    ...
    def test_family_geometry_matches_fixture(self) -> None:
        ...
        self.assertEqual(
            [row["name"] for row in report],
            [row["name"] for row in expected_families],
        )
        ...
        for expected in expected_families:
            ...
            for key, expected_value in expected.items():
                ...
                self.assertEqual(
                    actual_value,
                    expected_value,
                    msg=f"legacy SHEET_COL family audit mismatch for {expected['name']} {key}",
                )
    ...
    def test_basis_hashes_and_non_persistence_match_fixture(self) -> None:
        ...
        self.assertEqual(
            basis_sha256(trajectory.depth_basis),
            expected_hashes["depth_4_order_3_float32"],
        )
        self.assertEqual(
            basis_sha256(trajectory.row_basis("attention_input_weight")),
            expected_hashes["row_16_order_8_float32"],
        )
        self.assertEqual(
            basis_sha256(trajectory.row_basis("attention_input_bias")),
            expected_hashes["row_48_order_24_float32"],
        )
        self.assertEqual(
            basis_sha256(trajectory.row_basis("mlp_contraction_weight")),
            expected_hashes["row_64_order_32_float32"],
        )
    ...
    def test_selected_materializations_match_fixture(self) -> None:
        ...
        for name, expected in self.fixture["selected_materializations"].items():
            ...
            expected_values = torch.tensor(
                expected["first_values"],
                dtype=materialized.dtype,
                device=materialized.device,
            )
            torch.testing.assert_close(
                materialized.flatten()[: expected_values.numel()],
                expected_values,
                rtol=5.0e-6,
                atol=5.0e-7,
                msg=f"first materialized values changed for {name}",
            )
            torch.testing.assert_close(
                materialized.sum(),
                torch.tensor(expected["sum"], dtype=materialized.dtype),
                rtol=5.0e-6,
                atol=5.0e-7,
                msg=f"materialized sum changed for {name}",
            )
            torch.testing.assert_close(
                materialized.norm(),
                torch.tensor(expected["norm"], dtype=materialized.dtype),
                rtol=5.0e-6,
                atol=5.0e-7,
                msg=f"materialized norm changed for {name}",
            )

    def test_direct_value_matches_materialize_for_sampled_matrix_entries(self) -> None:
        trajectory = self.trajectory()
        samples = (
            ("attention_input_weight", 2, 0, 0),
            ("attention_input_weight", 2, 17, 9),
            ("attention_output_weight", 2, 4, 12),
            ("mlp_expansion_weight", 2, 63, 15),
            ("mlp_contraction_weight", 2, 15, 63),
        )
        for name, layer_index, output_row, row_index in samples:
            ...
            torch.testing.assert_close(
                direct,
                materialized[output_row, row_index],
                rtol=1.0e-6,
                atol=1.0e-7,
                msg=f"direct materialization path changed for {name}",
            )
    ...
    def test_current_attention_head_packing_audit_matches_fixture(self) -> None:
        ...
        attention_input_shape = trajectory.family_metadata(
            "attention_input_weight"
        ).coefficient_shape(config.depth_order)
        attention_output_geometry = trajectory.family_metadata(
