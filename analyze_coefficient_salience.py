...
from sheet.checkpoints import load_payload, strip_compiled_prefix
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("checkpoint", type=Path, help="THOG checkpoint, normally checkpoints/<artifact>/ckpt.pt")
    parser.add_argument("--data-dir", type=Path, default=Path("data/openwebtext"), help="dataset directory containing val.bin")
    parser.add_argument("--scope", default="depth", help="depth, one coefficient family, or a comma-separated family list")
    parser.add_argument("--list-scopes", action="store_true", help="load the checkpoint, list ablatable coefficient families, and exit")
    parser.add_argument("--device", default="auto", help="auto, cpu, cuda, cuda:0, ...")
    parser.add_argument("--dtype", choices=("auto", "float32", "float16", "bfloat16"), default="auto", help="evaluation autocast dtype")
    parser.add_argument("--eval-shards", type=int, default=4, help="fixed validation batches used as independent salience shards")
    parser.add_argument("--batch-size", type=int, default=2, help="sequences per validation shard")
    parser.add_argument("--gradient-shards", type=int, default=1, help="leading validation shards used for gradient diagnostics; 0 disables them")
    parser.add_argument("--seed", type=int, help="validation sampling seed; defaults deterministically from the checkpoint data seed")
    parser.add_argument("--output-prefix", type=Path, help="output path without extension; default is <checkpoint_dir>/salience/<scope>_u<updates>")
...
def _batch_from_starts(
    ...
    inputs = torch.stack(
        [torch.from_numpy(np.asarray(tokens[start : start + block_size], dtype=np.int64).copy()) for start in starts]
    )
    targets = torch.stack(
        [torch.from_numpy(np.asarray(tokens[start + 1 : start + block_size + 1], dtype=np.int64).copy()) for start in starts]
    )
    if device.type == "cuda":
        return inputs.pin_memory().to(device, non_blocking=True), targets.pin_memory().to(device, non_blocking=True)
...
def _print_scope_listing(banks: Sequence[CoefficientBank]) -> None:
    print("depth")
    for bank in banks:
        print(f"  {bank.name:<28} axis={bank.order_axis}  shape={tuple(bank.parameter.shape)}")
...
def _print_summary(rows: Sequence[Dict[str, Any]], statistics: Dict[str, Any]) -> None:
    ...
    for row in rows:
        ...
        print(f" {row['order']:>5d}  {row['delta_loss_zero']:>+14.7e}  {row['coefficient_rms']:>12.3e}  {grad_text:>12}")
...
def main() -> int:
    ...
    if not arguments.checkpoint.is_file():
        raise FileNotFoundError(f"checkpoint is missing: {arguments.checkpoint}")
    ...
    all_banks = discover_depth_banks(model)
    ...
    if any(bank.order_count != order_count for bank in banks):
        raise ValueError("selected coefficient banks do not share one order count")
    ...
    print(f"families:   {', '.join(bank.name for bank in banks)}")
    ...
    for order in range(order_count):
        ...
        delta = tuple(ablated_loss - baseline for ablated_loss, baseline in zip(ablated, baseline_by_shard))
        ablated_by_order.append(ablated)
        delta_by_order.append(delta)
        print(f"[{order + 1:>{len(str(order_count))}d}/{order_count}] order {order:>3d}: delta_loss={_mean(delta):+.7e}", flush=True)
    ...
    completed_updates = int(payload.get("completed_updates", payload.get("trainer_state", {}).get("completed_updates", 0)))
    ...
    csv_path, json_path = _write_outputs(prefix, rows, metadata, arguments.eval_shards, arguments.overwrite)
