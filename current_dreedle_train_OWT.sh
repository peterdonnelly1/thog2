...
# vvv THOG
# Current dreedle OWT train wrapper. Defaults: float16, attention_backend=sdpa.
# Presets: legacy_sheet_col, curve, mlp_block, head_aware_block, block. Basis: chebyshev, dct.
# Explicit module overrides stay reachable through -a attention_geometry and -m mlp_geometry.
# ^^^ THOG

cd "$(dirname "$0")"
HOST=dreedle DTYPE=float16 BACKEND=sdpa DATA_DIR="${THOG2_OWT_DATA_DIR:-$HOME/git/thog/data/openwebtext}" RUN_NAME="" PRESET=curve BASIS=chebyshev ATTN="" MLP="" STEPS=250 BATCH=3 ACCUM=160 LAYERS=144 HEADS=12 EMBD=768 CTX=1024 P=32 Q=64 DRY=false
...
echo "dreedle OWT train: preset=$PRESET basis=$BASIS backend=$BACKEND dtype=$DTYPE run=$RUN_NAME"
