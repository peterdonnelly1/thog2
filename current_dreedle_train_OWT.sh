...
# vvv THOG
# Current dreedle OWT train wrapper. Defaults: float16, sdpa, non-tiny SHEET L144 H12 D768 C1024.
# Use -O dense for dense baseline; dense defaults to L12 H12 D768 unless those shape knobs are explicitly set.
# Compact knobs: -p preset, -B basis, -a attention_geometry, -m mlp_geometry.
# ^^^ THOG

cd "$(dirname "$0")"
MODEL_TYPE=sheet; HOST=dreedle; DTYPE=float16; BACKEND=sdpa; DATA_DIR="${THOG2_OWT_DATA_DIR:-$HOME/git/thog/data/openwebtext}"; RUN_NAME=""; PRESET=curve; BASIS=chebyshev; ATTN=""; MLP=""; STEPS=250; BATCH=3; ACCUM=160; LAYERS=144; HEADS=12; EMBD=768; CTX=1024; DEPTH_ORDER=32; BASE_ROW_ORDER=64; CHECKPOINT_SEGMENT_SIZE=12; DRY=false; LAYERS_SET=false; HEADS_SET=false; EMBD_SET=false
...
if [[ "$MODEL_TYPE" == dense ]]; then [[ "$LAYERS_SET" == false ]] && LAYERS=12; [[ "$HEADS_SET" == false ]] && HEADS=12; [[ "$EMBD_SET" == false ]] && EMBD=768; fi
BASIS_TAG=CHEBY; [[ "$BASIS" == dct ]] && BASIS_TAG=DCT
PRESET_TAG="${PRESET^^}"; [[ "$PRESET" == legacy_sheet_col ]] && PRESET_TAG=SHEET_COL
...
echo "dreedle OWT train: model=$MODEL_TYPE preset=$PRESET basis=$BASIS backend=$BACKEND dtype=$DTYPE run=$RUN_NAME L=$LAYERS H=$HEADS D=$EMBD C=$CTX batch=$BATCH accum=$ACCUM"
