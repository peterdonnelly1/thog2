...
# vvv THOG one instrumentation selector determines both backend and W&B mode; -M is now layer-dropout active-per-stratum
...
if (( ${#PRESET_VALUES[@]} > 1 || ${#BASIS_FAMILY_VALUES[@]} > 1 || ${#MLP_HIDDEN_COMPRESSOR_VALUES[@]} > 1 || ${#MLP_HIDDEN_GROUP_SIZE_VALUES[@]} > 1 || ${#O_DEPTH_VALUES[@]} > 1 || ${#BATCH_SIZE_VALUES[@]} > 1 || ${#LEARNING_RATE_CODE_VALUES[@]} > 1 )); then
  echo "dreedle OWT grid: p=${PRESET_VALUES[*]} B=${BASIS_FAMILY_VALUES[*]} MHC=${MLP_HIDDEN_COMPRESSOR_VALUES[*]} MHG=${MLP_HIDDEN_GROUP_SIZE_VALUES[*]} P=${O_DEPTH_VALUES[*]} b=${BATCH_SIZE_VALUES[*]} LR=${LEARNING_RATE_CODE_VALUES[*]}"
fi
